...
const CSS = `
  ...
  .admin-tab-btn:hover { background: var(--surface); color: var(--dark); }
  ...
  .admin-btn:hover { transform: translate(-2px, -2px); box-shadow: 5px 5px 0 var(--green); }
  .admin-btn:active { transform: translate(0, 0); box-shadow: 1px 1px 0 var(--dark); }
  .admin-btn-primary { background: var(--green); color: #fff; }
  .admin-btn-danger { background: #dc2626; color: #fff; border-color: #991b1b; box-shadow: 3px 3px 0 #991b1b; }
  .admin-btn-danger:hover { box-shadow: 5px 5px 0 #dc2626; }
  .admin-btn-ghost { background: transparent; color: var(--dark); box-shadow: none; border-color: var(--border); }
  ...
  .admin-modal {
    background: #fff; border: 2px solid var(--dark);
    border-radius: 24px; box-shadow: 12px 12px 0 var(--dark);
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        {/* Command Log */}
        <div style={{ background: '#0f172a', border: '2px solid #0f172a', borderRadius: 18, padding: 0, boxShadow: '6px 6px 0 #16a34a', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: 440 }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, color: '#4ade80', letterSpacing: '0.1em' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0' }}>
            {cmdLog.length === 0 ? (
              ...
              <div key={entry.id} style={{ padding: '6px 16px', fontFamily: 'var(--mono)', fontSize: 11, color: entry.type === 'error' ? '#f87171' : entry.type === 'success' ? '#4ade80' : '#94a3b8', display: 'flex', gap: 8 }}>
                <span style={{ color: '#475569', flexShrink: 0 }}>{entry.at}</span>
...
export default function NewAdminDashboard() {
  ...
  const statCards = [
    { title: 'Total Revenue', icon: IndianRupee, color: '#16a34a', bg: '#dcfce7', to: stats.revenue, prefix: '₹' },
    { title: 'Energy Supplied', icon: BatteryCharging, color: '#2563eb', bg: '#dbeafe', to: stats.energy, suffix: ' kWh', decimals: 1 },
    { title: 'Registered Users', icon: Users, color: '#7c3aed', bg: '#ede9fe', to: nonAdminUsers.length },
    { title: 'Chargers Online', icon: Activity, color: '#ea580c', bg: '#ffedd5', to: onlineCount },
  ];
    ...
    <div style={{ minHeight: '100vh', background: '#fff', fontFamily: 'var(--display)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-end', gap: 32 }}>
            ...
            <motion.div className="admin-hero-right" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.18 }}
              ...
              {[
                ...
                { label: 'Chargers Online', value: `${onlineCount}`, icon: Cpu, color: '#60a5fa', bar: stats.totalStations > 0 ? (onlineCount / stats.totalStations * 100) : 0 },
                { label: 'Total Stations', value: `${stats.totalStations}`, icon: Server, color: '#f472b6', bar: 100 },
              ].map((item, i) => (
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 32px 88px', position: 'relative', zIndex: 1 }}>
          ...
          {tab === 'overview' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
              ...
              <div className="admin-stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 48 }}>
                {statCards.map(({ title, icon: Icon, color, bg, to, prefix = '', suffix = '', decimals = 0 }, i) => (
                  <motion.div key={title} className="admin-neo-card"
              ...
              <div style={{ background: '#fff', border: '2px solid #0f172a', borderRadius: 24, boxShadow: '8px 8px 0 #0f172a', overflow: 'hidden' }}>
                <div style={{ padding: '22px 28px', borderBottom: '2px solid #0f172a', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fafafa' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: '#ffedd5', border: '2px solid #0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '3px 3px 0 #0f172a' }}>
                      ...
                      {loading ? Array.from({ length: 5 }).map((_, i) => (
                        ...
                        return (
                          <motion.tr key={u.id} className="admin-trow"
                            ...
                            <td style={{ padding: '16px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <div style={{ width: 38, height: 38, borderRadius: 10, background: av.bg, color: av.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--display)', fontWeight: 900, fontSize: 15, border: '2px solid #0f172a', boxShadow: '2px 2px 0 #0f172a', flexShrink: 0 }}>
                            ...
                            <td style={{ padding: '16px' }}>
                              <span style={{ fontFamily: 'var(--mono)', fontWeight: 900, fontSize: 13, color: u.totalBookings > 0 ? '#16a34a' : '#cbd5e1', background: u.totalBookings > 0 ? '#f0fdf4' : '#f8fafc', border: `2px solid ${u.totalBookings > 0 ? '#16a34a' : '#e2e8f0'}`, borderRadius: 8, padding: '4px 14px', display: 'inline-block', boxShadow: u.totalBookings > 0 ? '2px 2px 0 #16a34a' : 'none' }}>
                                {u.totalBookings}
                              </span>
                            </td>
                            <td style={{ padding: '16px' }}>
          ...
          {tab === 'logs' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
              <div style={{ background: '#0f172a', border: '2px solid #0f172a', borderRadius: 24, boxShadow: '8px 8px 0 #16a34a', overflow: 'hidden' }}>
                <div style={{ padding: '18px 24px', borderBottom: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: '#4ade80', letterSpacing: '0.08em' }}>
